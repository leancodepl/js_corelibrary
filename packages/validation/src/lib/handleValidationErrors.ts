...
export function handleValidationErrors<TAllErrors extends Record<string, number>, TInResult = never>(
  ...
  const handle: ValidationErrorsHandleFunc<TAllErrors, TInResult> = <THandledErrors extends keyof TAllErrors, TResult>(
    ...
    const foundErrors = validationErrorsToHandle.reduce(
      (prev, cur) => {
        const ves = validationErrors.filter(
          (ve): ve is SpecificValidationError<TAllErrors, THandledErrors> => ve.ErrorCode === errorCodesMap[cur],
        )
