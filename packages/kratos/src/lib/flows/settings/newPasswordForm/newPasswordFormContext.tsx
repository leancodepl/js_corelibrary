...
export function NewPasswordFormProvider({
  ...
  const newPasswordFormContextData = useMemo<NewPasswordFormContextData>(() => ({ newPasswordForm }), [newPasswordForm])

  return (
    <newPasswordFormContext.Provider value={newPasswordFormContextData}>{children}</newPasswordFormContext.Provider>
  )
}
