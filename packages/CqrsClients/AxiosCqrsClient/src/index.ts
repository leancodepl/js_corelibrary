import { ApiError, ApiSuccess, CommandResult } from "@leancode/cqrs-client-base";
...
export default function mkCqrsClient(cqrsEndpoint: string, tokenProvider: TokenProvider) {
    ...
    return {
        createQuery<TQuery, TResult>(type: string) {
            return (dto: TQuery) => apiAxios.post<TResult>("query/" + type, dto).then(r => r.data);
        },
        createCommand<TCommand, TErrorCodes extends { [name: string]: number }>(
            ...
            return (dto: TCommand) =>
                apiAxios.post<CommandResult<TErrorCodes>>("command/" + type, dto).then(r => r.data);
        },
    };
}
